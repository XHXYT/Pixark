...
class KVdbUtil {
  ...
  /**
   * 获取指定键的值（支持泛型、默认值及错误返回默认值）
   * @param key 键
   * @param defValue 如果键不存在或出错时返回的默认值
   * @returns Promise 返回指定类型的值
   */
  public async getData<T>(key: string, defValue: Uint8Array | string | number | boolean): Promise<T> {
    if (!this.kvStore) {
      throw new Error('KVStore is not initialized.');
    }
    try {
      const value = await new Promise((resolve, reject) => {
        this.kvStore!.get(key, (err, data) => {
          if (err) {
            // 如果获取失败（如键不存在 15100004），reject 以触发 catch 块
            reject(err);
          } else {
            // 成功则 resolve 数据
            resolve(data as T);
      ...
      console.info(`KVStore.get Succeeded in getting ${key}. Value: ${value}`);
      return value as T;
    } catch (error) {
      // 发生任何错误（包括键不存在）时，打印日志并返回默认值
      const err = error as BusinessError;
      console.error(`KVStore.get Failed to get key '${key}'. Code: ${err.code}, Message: ${err.message}`);
  ...
  async hasKey(key: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      this.kvStore!.get(key, (err) => {
        if (err) {
          // 如果错误码是 15100004 (KEY_NOT_FOUND)，说明key不存在
          // 此处直接返回 false 即可，不需要区分具体错误码，因为任何错误都意味着取不到值
          resolve(false);
        } else {
          resolve(true);
        }
...
/** 键值型数据库工具类封装 */
export const kvDbUtil = new KVdbUtil();